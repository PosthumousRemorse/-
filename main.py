...
def acception(call):
    ...
    if str(call.text) != 'Отклонить':
        if len(str(call.text).split('Принять заявку от id')) < 2:
            z = telebot.types.ReplyKeyboardRemove()
            bot.send_message(call.from_user.id, 'Жмите на высветившиеся кнопки!', reply_markup=z)
        else:
...
def response(message):
    ...
    if str(message.text) != 'Нет':
        ...
        try:
            ...
            z = telebot.types.ReplyKeyboardRemove()
            bot.send_message(message.from_user.id, 'Жмите на высветившиеся кнопки!', reply_markup=z)
    else:
